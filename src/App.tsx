...
export default function App() {
  ...
  const allowedFileTypes = '.pdf,.jpg,.jpeg,.png,.bmp,.tif,.tiff,.webp,.csv,.xlsx,.xlsm,.docx,.pptx,.txt,.md,.log,.json,.xml,.html,.htm,.yaml,.yml'

  const handleFiles = (files: FileList | File[]) => {
    const supported = /\.(pdf|jpe?g|png|bmp|tiff?|webp|csv|xlsx|xlsm|docx|pptx|txt|md|log|json|xml|html?|ya?ml)$/i
    const incoming = Array.from(files).filter(file => supported.test(file.name))

    if (incoming.length > 0) {
      // The current Flask endpoint accepts one file per request.
  ...
  const convertBackendFinding = (finding: any, index: number): PiiItem => {
    ...
    const classification = String(
      finding.risk ?? finding.severity ?? finding.classification ?? ''
    ).toLowerCase()

    const risk: PiiItem['risk'] =
      classification.includes('high') || classification.includes('critical')
        ? 'high'
        : classification.includes('low')
          ? 'low'
          : 'medium'

    const numericConfidence =
      Number(finding.confidence ?? finding.confidence_score ?? finding.score)
    ...
    const location =
      ...
      finding.line ??
      finding.source ??
      'Detected by backend scanner'

    const masked = value.length > 4
      ? `${'•'.repeat(Math.max(1, value.length - 4))}${value.slice(-4)}`
      : '••••'
    ...
    return {
      ...
      necessity: risk === 'high' ? 'Review Required' : 'Review Recommended',
      recommendation:
        finding.recommendation ??
        'Review this detected information and apply appropriate data minimization or protection.',
      action: risk === 'high' ? 'mask' : 'keep',
  ...
  const startScan = async () => {
    ...
    setScanError('')
    setPiiItems([])
    setScanItems([`Uploading ${file.name}…`])
    setScanProgress(10)
    navigate('scanning')
    ...
    try {
      ...
      setScanProgress(25)
      setScanItems([`Uploading ${file.name}…`, 'Connected to PII detection backend'])
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || data.message || `Scan failed (HTTP ${response.status}).`)
      }

      const findings = Array.isArray(data.findings) ? data.findings : []
      const converted = findings.map(convertBackendFinding)

      setPiiItems(converted)
      setScanItems(
        converted.length > 0
          ? converted.map(item => `${item.type} detected`)
          : ['Scan completed — no PII was detected']
      )
      setScanProgress(100)

      // Give the user a moment to see the completed scan before opening results.
      setTimeout(() => navigate('results'), 500)
    } catch (error) {
      console.error('PII backend scan error:', error)
      const message = error instanceof Error
        ? error.message
        : 'Unable to connect to the PII detection backend.'
          ...
          <div className="lg:col-span-2 bg-card border border-border rounded-2xl p-6">
            ...
            <div className="bg-secondary rounded-xl p-5 space-y-4 font-mono-data text-xs">
              <div className="text-center py-5">
                ...
                <div className="font-semibold text-foreground text-sm break-all">{selectedFiles[0]?.name || 'Uploaded file'}</div>
                <div className="text-muted-foreground mt-2">
                  {selectedFiles[0] ? `${(selectedFiles[0].size / 1024).toFixed(1)} KB` : 'File uploaded'}
                </div>
              </div>
              <div className="border-t border-border pt-3 space-y-2">
                <div className="flex justify-between gap-3">
                  <span className="text-muted-foreground">Backend status</span>
                  <span className="text-green-700 font-medium">Scan complete</span>
                </div>
                <div className="flex justify-between gap-3">
